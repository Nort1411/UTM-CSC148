...
class RandomPlayer(Player):
    def move(self, current, min_step, max_step, goal):
        return randint(min_step, min (goal-current, max_step))

class StrategicPlayer(Player):
    def move(self, current, min_step, max_step, goal):
        jump = max_step + min_step
        target = goal - ((goal - current) // jump) * jump
        if target <= current:
            return min_step
...
class UserPlayer(Player):
    def move(self, current, min_step, max_step, goal):
        return int (input(f"Choose a number between {str(min_step)} and {str(max_step)}."))
...
def make_player(generic_name: str) -> Player:
    ...
    type_player = input(f'Enter the type of player for {generic_name}: ')
    if 'Strategic' in type_player:
        return StrategicPlayer(name)
    elif 'Random' in type_player:
        return RandomPlayer(name)
    elif 'User' in type_player:
